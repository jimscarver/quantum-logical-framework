...
| `QLF_PhysicalPi` | **π is generated by the closure census, not a primitive** (Allen's #86). π's generator is the substrate's *own* closure walk: the census of ZFA-balanced stable closures of length `2n` is `C(2n,n)` (`closure_census`, reusing `find_stable_states_length_even`), so the 2-D closure-walk return density `P₂ₙ(0) = (C(2n,n)/4ⁿ)²` is a finite computable **rational** (`returnDensity`, `returnDensity_eq_census`) with `n·P₂ₙ(0) → 1/π` ⟹ `π = lim 1/(n·returnDensity n)` — π from pure counting, no circle, no `Real.pi`. Same `C(2n,n)` as Born stats, the P-vs-NP verify-filter, and the Riemann gap-zero density, so `π` and `ζ` (via `π²=6ζ(2)`) run on one census. Closure *periods* `2π`/`4π`/`2π/3` = the `U(1)`/`SU(2)`/`SU(3)` gauge sectors. **Honest scope:** convergence (Wallis/Stirling) cited not formalized; the *geometric* bridge (discrete isotropy → circle, no circle premise) open (`physical_pi_in_progress`); `Real.pi` reframed as the machines' limit, not eliminated. See `Physical_Pi.md` |
