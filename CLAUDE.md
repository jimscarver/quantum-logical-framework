...
| `censusTail_eq` | `QLF_AlphaBound` | Analysis boundary: the central-binomial generating function `∑ C(2n,n)xⁿ = (1−4x)^(−1/2)` (a classical theorem not yet in this Mathlib), specialized at `x=1/128` to give the exact total-census α-screening tail `512√62/31 − 130`. The tail is *defined* constructively from the census counts; the `α⁻¹ < (217+512√62)/31` cap and `codata_below_alphaInvCap` are **proved** around it |
