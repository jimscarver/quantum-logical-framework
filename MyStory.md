...
- **Predicted physical constants emerge** directly from the single ZFA postulate with no fitting parameters, including:  
  ...
  • Fine-structure constant **α ≈ 0.007299 (≈ 1/137)** by two independent routes: (a) from the **ionization energy of hydrogen + electron rest energy** via the QLF Bohr derivation `α = sqrt(2 Ry / m_e c²)`, to CODATA precision (10⁻¹⁰ relative error); (b) from **QLF substrate combinatorics alone, no observable input**, via `α_QLF = 1/128 × (1+9α)⁻¹ = 1/137.000`, matching CODATA at 0.026% from the 8-twist alphabet + N=9 directional modes. See [`Hydrogen.md`](Hydrogen.md) §4, [`Magnetism_Spatial_Dynamics.md`](Magnetism_Spatial_Dynamics.md) §6.1, [`fine_structure_demo.py`](fine_structure_demo.py), [`magnetism_spatial_dynamics_demo.py`](magnetism_spatial_dynamics_demo.py).  
