...
- **`B−L` conservation** — the *exact* baryonic/leptonic symmetry. The signed twist count that ZFA balances per event is `B−L` (the difference), not `B` or `L` separately: composite hadron blankets ([Hadrons_Markov_Blankets.md](Hadrons_Markov_Blankets.md)) carry a quantized topological-winding number (`B`), and the electron's chiral fluxoid carries the lepton count (`L`, [Atom.md](Atom.md)), but only their `B−L`-balanced combination is protected per event. This is what predicts the neutrino is **Dirac** and forbids `0νββ` ([Beta_Decay_Neutrino_Nature.md](Beta_Decay_Neutrino_Nature.md) §1): `0νββ` would be a `Δ(B−L)=2` imbalance. `B` and `L` may each shift in a `B−L`-conserving (sphaleron / baryogenesis) process — consistent with the Sakharov `B`-violation needed for matter dominance (§7). **What is Lean-anchored, and what is not, is now sharp** ([`lean/QLF_BMinusL.lean`](lean/QLF_BMinusL.lean)): `signed_count_conserved` proves any *signed twist count* (electric charge is the instance) is conserved by the ZFA dynamics, while `wcount_zero_on_ZFA` proves every such count is **zero on every closure**. Charge passes; `B−L` does **not** — a neutron is neutral (`Q=0`) yet carries `B−L=1`, and a baryon vs antibaryon are count-balanced closures with the identical twist multiset (`countBalanced` is per-axis) yet `B−L = ±1`, which no count-function can distinguish. So **`B−L` is not a signed count — it is a winding / orientation invariant** (sequence-dependent), exactly the "topological-winding" baryon number above. Its conservation is therefore a *winding* invariant of the dynamics — the correct but **still-open** Lean target; the weight-dictionary route is now provably closed. (This corrects an earlier note that called `B−L` conservation "the same theorem as charge conservation.") Notably, `B−L` conservation is predicted to hold **even across black-hole evaporation** — a QLF black hole returns its `B−L` bit-for-bit via Hawking radiation (§6) — which **diverges from the mainstream quantum-gravity expectation** that gravity admits no exact global symmetries. QLF's gravity is emergent (not fundamental), so it cannot violate what ZFA enforces; see [Quantum_Gravity.md §8](Quantum_Gravity.md).
