...
| **Entanglement / nonlocality (ER=EPR)** | Entanglement is a shared **primordial closure** — two conjugates that fold to identity (the singlet, `opposite_spin_singlet_closes`). Since geometry is the *rendering* of closures, that shared closure rendered geometrically **is** a non-traversable Einstein–Rosen bridge: ER=EPR is *forced*, not conjectured. Non-locality dissolves — the two ends are one closure that space is synthesized *around* (the closure is prior to space), so nothing is transmitted *through* the geometry (`no_ftl_in_epr`) | ✅ concept (QLF-native) / 🟣 Lean tie to core open | [`Primordial_Entanglement.md`](Primordial_Entanglement.md), [`ER_EPR_QLF.md`](ER_EPR_QLF.md), `ER_EPR_QLF.lean` |
