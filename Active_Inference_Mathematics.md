...
| **Quantitative mass spectrum** | gauge-fold depth → mass; falsifications: `lepton_mass_demo.py` (naive M(N) growth wrong shape), `lepton_mass_prime_test.py` (one partial hit m ∝ p_k! on m_τ/m_μ within 19%, otherwise wrong), `lepton_mass_irreducible_test.py` (totally-irreducible-at-prime-depth filter: 48/912/22944 nested family identified at N=4/6/8; M_irr ratios 19, 25 still monotonic, lepton 207/17 non-monotonic — the right derivation needs an ingredient orthogonal to both multiplicity AND irreducibility) | ✗ Open (constrained by three falsifiable demos) |
