...
| **Quantitative mass spectrum** | atomic-system masses (positronium, muonium, hydrogen) as the QLF observables per [`Bound_States_QLF.md`](Bound_States_QLF.md); free-lepton ratios are not direct QLF observables; per-qubit mass quantum `m_qubit c² = ℏω = E_Planck / R_qubit` per [`Per_Qubit_Mass_Quantum.md`](Per_Qubit_Mass_Quantum.md); photon-side per-bit energy quantum per [`Photon_Energy_Bits.md`](Photon_Energy_Bits.md) (mass-equivalence `E/c²`, zero rest mass) | ⚠ Structure derived (per-qubit principle reproduces all measured mass ratios exactly; photon energy & mass-equivalence as bit count × `ℏω`); first-principles `R_e` derivation still open |
