...
Together these three demos pin down what the eventual derivation cannot be: a pure monotonic function of multiplicity, depth, or prime arithmetic alone. **A more important reframing**: per [`Bound_States_QLF.md`](Bound_States_QLF.md), free-lepton mass ratios are not direct QLF observables. The natural QLF observables are atomic-system masses (positronium, muonium, hydrogen) — bound, balanced, joint-ZFA closures. The three demos correctly falsified "M(N) → free-lepton mass" formulas, but the right target is atomic-system mass instead. The τ has no stable atomic system; its mass corresponds to a decay-vertex closure rather than a Bohr-bound state.
