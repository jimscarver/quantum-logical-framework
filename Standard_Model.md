...
- **Mass ratios from multiplicity**: pursue the quantitative bridge from Energy_Combinatorics to specific mass values, scoped per [`Bound_States_QLF.md`](Bound_States_QLF.md) to atomic-system masses (positronium 1.022 MeV, muonium 106.17 MeV, hydrogen 938.78 MeV). [`Atomic_System_QLF_Closures.md`](Atomic_System_QLF_Closures.md) writes out the joint-closure mapping for each atomic system and derives the Bohr reduced-mass scaling `E(Mu)/E(Ps) ≈ 2`, `E(H)/E(Mu) ≈ 1`. The first-principles derivation of `α R_e = m_e` from QLF closure-multiplicity is the remaining open piece; scope and three candidate pathways are documented in [`Electron_Mass_Derivation.md`](Electron_Mass_Derivation.md), which also reports an empirical test showing the existing `constants_mapper.emerge_alpha()` returns 0.2 (gauge/spatial ratio of the small stable-history ensemble) rather than the fine-structure constant 1/137.
