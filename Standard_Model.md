...
- **Mass ratios from multiplicity**: scoped per [`Bound_States_QLF.md`](Bound_States_QLF.md) to atomic-system masses (positronium 1.022 MeV, muonium 106.17 MeV, hydrogen 938.78 MeV). [`Atomic_System_QLF_Closures.md`](Atomic_System_QLF_Closures.md) writes out the joint-closure mapping for each atomic system and derives the Bohr reduced-mass scaling `E(Mu)/E(Ps) ≈ 2`, `E(H)/E(Mu) ≈ 1`. The per-qubit reading [`Per_Qubit_Mass_Quantum.md`](Per_Qubit_Mass_Quantum.md) gives `m_qubit c² = ℏω = E_Planck / R_qubit` and reproduces all measured mass ratios via `m_X/m_Y = R_Y/R_X` exactly. The open piece is the first-principles derivation of the depth `R_e ≈ 2.4 × 10²²` from QLF closure-multiplicity, equivalent to deriving `α R_e = m_e` under the unit identification.
