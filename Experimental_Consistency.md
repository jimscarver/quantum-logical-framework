...
- `hierarchy_log_eq_fourteen_pi` — the Planck/proton hierarchy `= 14π` from the single integer 7 (QLF_AlphaS.lean), with `QLF_MassSpectrum`/`QLF_AlphaS` reducing the whole spectrum to one scale; the **bounds pass** `hierarchy_log_band` / `measured_hierarchy_in_band` / `hierarchy_band_width` makes the `α_s` exp-sensitivity explicit (the `0.07%` is the agreement at the posit `α_s=1/b₀²`; the running-consistent `1/α_s∈[49,52]` gives the log-band `[14π, 104π/7]`, measured 44.01 inside near the `14π` edge)
