...
| **a₀** (dark-matter / MOND scale) | g† = 1.20 × 10⁻¹⁰ m/s² (McGaugh RAR 2016) | Crossover acceleration `a₀ = cH₀/(2π) = c²/(2π R_H)` — the de Sitter horizon acceleration on the **same Hubble radius R_H** that fixes `Ω_Λ = log 2`. **Lean-verified** as `mond_acceleration_horizon_form` in `QLF_DarkMatter.lean`. Dark matter = denser logic inside the transition radius `σ = √(GM/a₀)`; same scale gives baryonic Tully–Fisher `v⁴ = GMa₀` | **−13%** at Planck H₀=67.4 (pred 1.04×10⁻¹⁰), **−6%** at local H₀=73 (1.13×10⁻¹⁰) — both inside the RAR ~20% systematic. H_0 as observable; see [`DarkMatter.md`](DarkMatter.md) §5 |
| **Planck/proton mass hierarchy** | `ln(M_Planck/m_p) ≈ 44.01` (the 10¹⁹ hierarchy) | Dimensional transmutation collapses to a **pure integer**: `ln R_p = 2π/(b₀ α_s) = 2π·b₀ = 14π` with the substrate QCD `b₀ = 7` (= `N_c=3`, `n_f=6`) and `α_s = 1/b₀²`. **Lean-verified** `hierarchy_log_eq_fourteen_pi` (`QLF_AlphaS.lean`), `beta_coefficient_eq_seven` (`QLF_BetaFunction.lean`) | **0.07%** on the log (`14π ≈ 43.98`); the whole mass scale from the single integer `7`. Value-level ~3% is Planck-mass/SI calibration ([`Per_Qubit_Mass_Quantum.md`](Per_Qubit_Mass_Quantum.md)) |
...
| **Koide relation `Q = 2/3`** → **m_τ** | Q = 0.666661 (PDG); m_τ = 1776.86 MeV | `Q = 2/3` forced by `N=3 ∧ A²=2` (three 120° phases); given m_e, m_μ it predicts m_τ. **Lean-verified** `koide_two_thirds` / `koide_three_phase` in `QLF_Koide.lean` | **0.006%** on m_τ (zero free parameters) |
...
| **Hadronic depth** | geometric cosmic depth `n ≈ 6.7 × 10⁶⁰` (primordial-blanket `v(R_H) ≈ R_H/l_P`) gives cosmic age (13.8 Gyr) and size (~10²⁶ m); the proton cube `(m_Planck/m_p)³ ≈ 2.2×10⁵⁷` reproduces it only to ~3–4 orders (large-number coincidence, not precise) | HadronicDepth.md §2.1, AgeOfUniverse.md §4.1 |
...
### Lean-verified theorems (71 active modules, zero `sorry`)
...
- **`mond_acceleration_horizon_form` — dark-matter / MOND acceleration scale Lean-anchored (QLF_DarkMatter.lean)**: the crossover acceleration `a₀ = cH₀/(2π) = c²/(2π R_H)` sits on the **same Hubble radius `R_H = c/H₀`** that fixes `Ω_Λ = log 2` (so dark matter and dark energy close on one horizon). Transition radius `mond_radius_accel : GM/σ² = a₀` with `σ = √(GM/a₀)`; dense/sparse crossover `newtonian_dominates_iff : a₀ < GM/r² ⟺ r² < GM/a₀` (dense interior = Newton/GR — Mercury, hadron quantum black holes; sparse exterior = apparent dark matter); baryonic Tully–Fisher `tully_fisher_flat : v⁴ = GMa₀`; Gaussian MRE congestion bump `gaussian_denser_near_center`. Demo: a₀ = 1.04×10⁻¹⁰ (Planck H₀) / 1.13×10⁻¹⁰ (local H₀) vs McGaugh RAR g† = 1.20×10⁻¹⁰ m/s² → **−13% / −6%, both inside the RAR ~20% systematic**. Honest scope: `cH₀` scale principled, the `1/2π` prefactor and full rotation curve open (`dark_matter_acceleration_scale_in_progress`); the Gaussian is the transition bump, not the flat `1/r²` tail
...
- `hierarchy_log_eq_fourteen_pi` — the Planck/proton hierarchy `= 14π` from the single integer 7 (QLF_AlphaS.lean), with `QLF_MassSpectrum`/`QLF_AlphaS` reducing the whole spectrum to one scale
...
*This list is illustrative; the authoritative, always-current table of all 71 modules and their key theorems is [`lean/README.md`](lean/README.md).*
...
Empirically, **every count-balanced history is automatically Pauli-closed** in the QLF Python BFS ensemble at every length tested. This is **now a Lean theorem** (`count_balanced_pauli_closed`, below): count balance alone implies Pauli closure, so the second conjunct of the runtime check is entailed by the first — the explicit enforcement formalizes an invariant that is provably present, not merely observed.

**Lean status (Pauli closure now fully general).** The two algebraic kernels of the runtime `is_zfa = is_count_balanced ∧ is_pauli_closed` check are Lean-verified, and Pauli closure is now proven for arbitrary histories (not just under concatenation):
...
71 active Lean modules; zero `sorry` blocks. The QuantumOS runtime exposes the QLF adjoint (Hermitian conjugation, the structural "negation" operator) as the `/conj <twists>` slash command — letting users construct and probe `Σ_sa = {H : H = H†}`, the operator-side counterpart of the Riemann ξ critical line ([ReverseMathematics.md](ReverseMathematics.md) §4.9).
...
This is coherent with active research lines (Verlinde's entropic gravity, holographic-screen approaches) and has since produced **quantitative, Lean-anchored** results: Newton's `1/r²` from the holographic event count (`QLF_GravityFromDelay`), the **Mercury perihelion at 42.99″/century, 0.03%** (`QLF_MercuryPerihelion`, the canonical test, now closed), the cosmological constant `Ω_Λ = log 2` at 1.2% (`QLF_CosmologicalConstant`), and the **dark-matter / MOND acceleration scale `a₀ = cH₀/(2π)`** at −13%/−6% (`QLF_DarkMatter`) — the last three all closing on the *same* Hubble horizon `R_H`. What remains genuinely open is the absolute `G` in SI units (the 37% `G_prediction_SI` residual in §6.3 is a calibration gap, not a derivation) and the dark-matter rotation-curve *profile* (the `a₀` scale and Tully–Fisher are anchored; the full bump-plus-`1/r²`-tail curve and the exact `1/2π` prefactor are `dark_matter_acceleration_scale_in_progress`).
...
| **Dark-matter / MOND scale `a₀ = cH₀/(2π)`** (§1.1, [DarkMatter.md](DarkMatter.md) §5) | Predicted 1.04–1.13×10⁻¹⁰ m/s² (Planck–local H₀) vs RAR `g† = 1.20×10⁻¹⁰`; −13% to −6%, inside the RAR ~20% systematic | QLF *locks* the dark-matter acceleration scale to the **same Hubble horizon** as `Ω_Λ`. If a precise, systematics-controlled `a₀` settles well outside `cH₀/(2π)` (beyond the O(1)-prefactor band), or if `a₀` is shown to track `H(z)` differently from the de Sitter horizon, the single-horizon dark-sector closure is refuted. Standard ΛCDM does not predict any `a₀`–`H₀` link, so this is a genuine QLF-specific test |
