...
## §9 Beta Decay and the Nature of the Neutrino (Dirac → no 0νββ)

In QLF beta decay, a neutron's topologically stressed Markov blanket ejects an electron (chiral ZFA loop, `L=+1`) and a **Dirac antineutrino** (`L=−1`) — born as a balanced lepton-number pair, exactly as the electron's charge is co-produced to complete the proton-deficit into a neutral hydrogen-class closure. The neutrino's spatial loop is non-chiral, but that does **not** make it self-conjugate: the antineutrino is the distinct conjugate, carrying the opposite conserved lepton count.
...
| **Neutrinos are Dirac → no 0νββ** (`B−L` exactly conserved) | Neutrinoless double-beta-decay searches (LEGEND, nEXO, KamLAND-Zen) — all null to date, consistent | Standard Model is agnostic (Dirac vs Majorana is open). A confirmed **positive** 0νββ signal would refute QLF's exact-`B−L` (per-event signed-count balance) account specifically, without affecting the rest of standard physics |
