...
- **α** numerically: the QLF Bohr derivation in [Hydrogen.md](Hydrogen.md) §4 gives `E_1 = −(1/2) α² m_e c²`, which inverts at the measured Rydberg to `α = sqrt(2 Ry / m_e c²) = 0.0072973526` (1/α = 137.036), agreeing with CODATA to 10⁻¹⁰ relative error. Equivalently in QLF Planck units via the per-qubit relation `m_e c² = E_Planck / R_e`: `α² = 2 Ry R_e / E_Planck`. See [`fine_structure_demo.py`](fine_structure_demo.py). This is α from measured Ry and measured m_e (or equivalently the calibrated R_e ≈ 2.4 × 10²²), **not** α from first principles. The remaining first-principles questions: derive Ry from the hydrogen joint-closure topology without using α as input; derive R_e from QLF closure-multiplicity at the Planck-event scale (the natural reformulation under the per-qubit reading of [Per_Qubit_Mass_Quantum.md](Per_Qubit_Mass_Quantum.md) §3.3).
...
- **Fine-structure constant α numerically** via QLF Bohr inversion of the hydrogen spectrum: `α = sqrt(2 Ry / m_e c²) = 0.0072973526` to 10⁻¹⁰ relative error vs CODATA. Equivalently `α² = 2 Ry R_e / E_Planck` in QLF Planck units. From measured Ry and m_e (calibrated R_e), not first-principles; first-principles Ry and R_e remain open. See [`fine_structure_demo.py`](fine_structure_demo.py) and [Hydrogen.md](Hydrogen.md) §4.
