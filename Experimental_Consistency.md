...
| Speed of light c | Cosmic-ratio identity `c = R_cosmic / T_cosmic = L_Planck / τ_Planck`; the cosmic-horizon depth `n` cancels exactly, and the same ρ-cancellation gives `c_local = c_substrate` at any Markov-blanket depth | 299 792 458 m/s | Scale-invariance structurally derived ([Kitada_Local_Time_GR.md](Kitada_Local_Time_GR.md) §5.3, [lean/QLF_SubstrateLightSpeed.lean](lean/QLF_SubstrateLightSpeed.lean)). Numerical c is measured — the cosmic-ratio identity is tautological at the numerical level once Planck units are defined in terms of {ℏ, G, c}; what is derived is the frame-invariance of the ratio |
...
Implementation: [SpaceTime.md](SpaceTime.md), `path_integral.py`. The substrate identity `c = L_Planck / τ_Planck` is derived from the cosmic-ratio (`R_cosmic = n · L_Planck`, `T_cosmic = n · τ_Planck`, ratio independent of `n`); this is *scale-invariance* — what is QLF-derived is c's frame-independence, not its numerical value. Numerical c is measured (and folded into the definition of L_Planck and τ_Planck via {ℏ, G, c}). Lean anchor in [`lean/QLF_SubstrateLightSpeed.lean`](lean/QLF_SubstrateLightSpeed.lean).
...
- **Tier 3 (open):** numerical α from QLF closure-multiplicity alone with no observable input — reduces to deriving R_e (= R_p · 6π⁵, [Proton_Resonance_R_e.md](Proton_Resonance_R_e.md)).
...
- **α from the ionization energy of hydrogen** (three tiers):
  ...
  - **Tier 3 (first-principles open):** numerical α from QLF closure-multiplicity alone, with no observable input. Reduces to deriving R_e (equivalently R_p · 6π⁵, [`Proton_Resonance_R_e.md`](Proton_Resonance_R_e.md)) from closure structure. Proton_Resonance_R_e sharpens this via the chirality-hiding-resonance reading — the `6π⁵` Lenz factor (1951 coincidence to 0.002%) recovered as `|S_3| · π⁵` from `3!` quark permutation symmetry × 5-angle integration over the proton's hidden-chirality configuration space. Each sub-factor is still open in full quantitative form (specifically the 5-angle count from Borromean topology and `R_p` from gauge-fold-depth combinatorics), but both are sharper sub-problems than direct R_e enumeration.
