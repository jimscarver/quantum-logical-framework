...
private def ket0 : Form := { t := 1/2, x := 0,   y := 0, z :=  1/2 }
/-- |1⟩ as density matrix |1⟩⟨1| = [[0,0],[0,1]] -/
private def ket1 : Form := { t := 1/2, x := 0,   y := 0, z := -1/2 }
/-- |+⟩ as density matrix |+⟩⟨+| = [[½,½],[½,½]] -/
private def ket_plus : Form := { t := 1/2, x := 1/2, y := 0, z :=    0 }
...
private def ket_minus : Form := { t := 1/2, x := -1/2, y := 0, z := 0 }
