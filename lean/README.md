...
| [QLF_BSD.lean](QLF_BSD.lean) | **Birch–Swinnerton-Dyer** (Millennium Prize) via the Langlands hook: the L(E,s) central point `s=1` is the self-dual fixed point of `s↦2−s` (`bsd_central_point_self_dual`), and the rank identity rank = ord (modularity = the QLF Hermitian-pair mirror, Langlands.md §5.4) is the single explicit boundary axiom `bsd_rank_equals_order`, from which qualitative BSD `bsd_in_qlf` (`E(ℚ)` infinite ⟺ `L(E,1)=0`) is derived. Weakest-machinery fit; `EllipticCurveQLF`/ranks abstract pending the constructive curve encoding; status `bsd_proof_in_progress`. See [BSD_QLF.md](../BSD_QLF.md) | `bsd_central_point_self_dual`, `bsd_rank_equals_order`, `bsd_rank_eq_in_qlf`, `bsd_in_qlf`, `bsd_proof_in_progress` |
