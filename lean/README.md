...
**Status:** CI passing, zero `sorry` blocks across all 47 active modules
...
| [QLF_BSD.lean](QLF_BSD.lean) | **Birch–Swinnerton-Dyer** (Millennium Prize) via the Langlands hook: the L(E,s) central point `s=1` is the self-dual fixed point of `s↦2−s` (`bsd_central_point_self_dual`), now grounded in the *same* `H↔H†` involution as Riemann — both central loci are `a/2` midpoints of an `s↦a−s` reflection (`reflection_fixed_iff`, `bsd_central_point_eq_midpoint`, `bsd_riemann_shared_involution` reusing `functional_equation_fixed_real`). The rank identity rank = ord (modularity = the QLF Hermitian-pair mirror, Langlands.md §5.4) is the single explicit boundary axiom `bsd_rank_equals_order`, from which qualitative BSD `bsd_in_qlf` (`E(ℚ)` infinite ⟺ `L(E,1)=0`) is derived. **`EllipticCurveQLF` is now concrete** (integral Weierstrass `y²=x³+ax+b`) with its closure encoding *computed* — `affinePointCount`/`frobeniusTrace` (= `a_p = p − #E(𝔽_p)`) over `ZMod p`, worked curve `Ecn1` with `Ecn1_smooth` and verified `Ecn1_frobenius_two` (`a₂=0`); only the two ranks stay abstract (uncomputable in general — BSD's content). Status `bsd_proof_in_progress`. See [BSD_QLF.md](../BSD_QLF.md) | `bsd_central_point_self_dual`, `bsd_riemann_shared_involution`, `EllipticCurveQLF`, `discriminant`, `affinePointCount`, `frobeniusTrace`, `Ecn1`, `Ecn1_smooth`, `Ecn1_frobenius_two`, `bsd_rank_equals_order`, `bsd_in_qlf`, `bsd_proof_in_progress` |
| [QLF_Hodge.lean](QLF_Hodge.lean) | **Hodge conjecture** (Millennium Prize) as the cohomological face of ZFA selection: the Hodge conjugation `H^{p,q}↔H^{q,p}` IS the QLF adjoint involution H↔H† (`conj_involutive`), and Hodge classes are exactly its balanced self-dual fixed points — the `(p,p)` diagonal (`conj_fixed_of_isHodge`, `isHodge_of_conj_fixed`). The conjecture "every Hodge class is algebraic" = *balanced ⟹ realized*, the cohomological lift of the substrate theorem `count_balanced_pauli_closed`, reduced to the single boundary axiom `hodge_class_is_algebraic` (contrapositive `non_algebraic_not_hodge` derived). The *balanced ⟹ realized* pattern is embedded as the proven `hodge_pattern_substrate_witness` (= `count_balanced_pauli_closed` lifted to cohomology). `CohClass`/`isAlgebraic` abstract pending the constructive cohomology encoding; status `hodge_proof_in_progress`. See [Hodge_QLF.md](../Hodge_QLF.md) | `CohClass.conj_involutive`, `CohClass.isHodge`, `conj_fixed_of_isHodge`, `isHodge_of_conj_fixed`, `hodge_pattern_substrate_witness`, `hodge_class_is_algebraic`, `hodge_conjecture_in_qlf`, `non_algebraic_not_hodge`, `hodge_proof_in_progress` |
