...
- **Gravitational waves — partially anchored.** The static metric here does not give the *wave*. But several features of a GW now follow from existing substrate machinery and are Lean-anchored in [`lean/QLF_GravitationalWaves.lean`](lean/QLF_GravitationalWaves.lean): a GW carries no gauge fold, so it is **massless** and propagates at **`c = L_Planck/τ_Planck`** (`gw_speed_eq_planck_ratio` — the GW170817 result `|v_GW − c|/c < 10⁻¹⁵`); the **graviton is spin-2**, a composite of four half-spins = two photon-worths (`graviton_integer_spin`, reusing `boson_even_pairs` from [`QLF_Spin`](lean/QLF_Spin.lean)); and it has **2 transverse polarizations** (the `±2` helicities, not `2J+1=5`) as a masslessness consequence, exactly like the photon's 2 (`massless_two_polarizations`). What is **still open** (`gravitational_waves_in_progress`): the linearized wave equation `□h_μν = 0` and the quadrupole luminosity `L = (G/5c⁵)⟨d³Q/dt³⟩²` — these need the *dynamical* substrate metric (the same gap as the full Einstein field equations). The `G` and `8π` in the quadrupole formula are the substrate `G = L_P²c³/ℏ` and `8π = 4π·2`.
