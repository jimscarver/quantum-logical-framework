...
This is the **per-event** anchor of the unifying claim. The global selection rule — that every constructible RhoProcess is the trajectory of an agent maximising mutual information against the vacuum prior — is the next-tier theorem flagged in [`Active_Inference_Mathematics.md`](Active_Inference_Mathematics.md) §7; the per-event iff established here is its first formal anchor. Quantitative consequences (Standard-Model gauge groups, first-principles mass spectrum, sterile-sector predictions) remain open work the principle is meant to unblock.
