...
- **Physics is derived, not assumed.** From that one primitive follow spacetime, measurement, entanglement, the Standard-Model gauge groups, gravity, spin, and the fundamental constants — each **machine-verified in Lean 4** across **98 modules with zero `sorry` blocks**, the combinatorial core operating strictly within **RCA₀** (no Axiom of Choice, no continuity).
...
**It's live and collaborative.** The running implementation is **[quantum-os](https://github.com/jimscarver/quantum-os)** — [open a room](https://jimscarver.github.io/quantum-os/) in any browser (no install, no server): evaluate ZFA (`/qucalc`), vote and govern (`/poll`, `/gov`), invite trust-governed **AI agent members** (a facilitator, scribe, skeptic), and run the room as a **[collective optimizer](https://github.com/jimscarver/quantum-os/blob/main/Collective_Optimization.md)**. Start with its **[User Guide](https://github.com/jimscarver/quantum-os/blob/main/User_Guide.md)**.
...
The Lean formalization compiles with **zero `sorry` blocks** across all 98 modules — but *zero `sorry` is not zero assumption*: the load-bearing work for each Millennium problem is a named `axiom`. The combinatorial core operates within **RCA₀** (no Axiom of Choice, no continuity). Each of the six open Clay problems has a Lean module **reformulating** it as a verified discrete core plus *one* explicit bridge axiom (`spectral_hilbert_polya`, `yang_mills_continuum_gap`, `modularity_mirror_invariant`, `substrate_realization_is_algebraic`, `navier_stokes_continuum_limit`, `generate_not_reducible_to_verify`) — and for the *finitary* problems that bridge is **of full conjecture strength**, so `bsd_rank_equals_order` and `hodge_class_is_algebraic` are *derivations from the conjecture-as-axiom — reformulations, not proofs*. QLF does **not** claim to prove the Millennium problems; the defensible claim is the substrate reformulation and the ontology behind it (see [Millennium.md](Millennium.md)). Plus philosophical axioms in `ER_EPR_QLF` (not used by other modules).
