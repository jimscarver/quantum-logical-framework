...
- [**Bound_States_QLF.md**](Bound_States_QLF.md) — free leptons are not QLF observables; the natural QLF observables are atomic systems (positronium, muonium, hydrogen) — bound, balanced, joint-ZFA closures. Reframes the lepton-mass-spectrum question and explains why the three lepton-mass demos (`lepton_mass_demo.py`, `lepton_mass_prime_test.py`, `lepton_mass_irreducible_test.py`) all failed by asking about a non-observable
