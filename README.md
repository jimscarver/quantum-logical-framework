...
- [**Atomic_System_QLF_Closures.md**](Atomic_System_QLF_Closures.md) — specific joint-closure topology for each atomic system: positronium (symmetric two-electron-half-loops, mass 2 m_e), hydrogen (electron half-loop + proton internal closure, mass dominated by m_p), muonium (asymmetric electron + antimuon half-loops). Derives the Bohr reduced-mass scaling `E(Mu)/E(Ps) ≈ 2`, `E(H)/E(Mu) ≈ 1` from the joint-closure structure. First quantitative QLF derivation on the right atomic-system targets
- [**Electron_Mass_Derivation.md**](Electron_Mass_Derivation.md) — scope and gaps for the first-principles derivation of `m_e` from QLF closure-multiplicity. Empirical test: existing `constants_mapper.emerge_alpha()` returns 0.2, not the fine-structure 1/137 — the chain "QLF → α → m_e via Bohr" does not work today. Three candidate pathways forward (Path A: multiplicity-weighted gauge-fold-path counting; Path B: direct Planck-event-rate counting; Path C: m_e/m_p ratio from joint-closure topology)
