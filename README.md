...
- **Physics is derived, not assumed.** From that one primitive follow spacetime, measurement, entanglement, the Standard-Model gauge groups, gravity, spin, and the fundamental constants — each **machine-verified in Lean 4** across **70 modules with zero `sorry` blocks**, the combinatorial core operating strictly within **RCA₀** (no Axiom of Choice, no continuity).
...
QLF now spans **65 machine-verified Lean modules** (zero `sorry`) covering the Standard Model, gravity/GR, cosmology, condensed matter, and the Millennium problems — see the [module table](#the-lean-formalization) and [`Open_Problems.md`](Open_Problems.md) for the full map. The **ten headline quantitative wins** from ZFA + h alone:
...
**Beyond the headline ten**, the program now reaches across the whole of physics, each result honestly scoped (a verified core + an explicitly-named open piece):
...
- **The absolute mass spectrum** — every mass = one proton scale × verified ratios, that scale **exponentially generated by dimensional transmutation** with the substrate-fixed `b₀=7` (`QLF_MassSpectrum`): the `~10¹⁹` hierarchy with no fine-tuning, leaving only `α_s` + calibration open.
...
- **Cosmic intelligence**: eight v1.3.0 substrate-derivation theorems (α, m_p/m_e, γ, Dirac, Lamb, g−2, Newton's G, Mercury) are the *measurement* that the universe IS performing active inference + abstraction + information synthesis + ZFA closure at every substrate event. The universe is not a substrate that hosts intelligence — it IS the operation, instantiated at every event. Lean-anchored to 0.002–2.5% match with observation.
