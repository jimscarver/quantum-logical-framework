...
- [**Electron_Mass_Derivation.md**](Electron_Mass_Derivation.md) — scope and gaps for the first-principles derivation of `m_e` from QLF closure-multiplicity. Empirical test: existing `constants_mapper.emerge_alpha()` returns 0.2, not the fine-structure 1/137 — the chain "QLF → α → m_e via Bohr" does not work today. Three candidate pathways: Paths A and C tested and falsified in their simple forms (alpha_emergence_scan.py and electron_proton_ratio_test.py); Path B untested
