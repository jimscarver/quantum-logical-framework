...
- [**genesis.py**](genesis.py) — the ZFA landscape spectrum explorer (exploratory instrument, every result epistemically tagged): the *exact* `−p/2` census spectral exponent that counts conjugate pairs, census → π, the `128 + d²` joint — with the honest negatives at full weight (the raw swap-graph dimension **diverges**, no discrete-scale-invariance in the binary sector, the α residual out). Full annotated sample run: [**Genesis.md**](Genesis.md)
