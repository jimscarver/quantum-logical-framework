...
The current Lean file [`lean/QLF_Riemann.lean`](lean/QLF_Riemann.lean) uses the following structure.
...
Two bridge statements are introduced in [`lean/QLF_Riemann.lean`](lean/QLF_Riemann.lean):
...
The final theorem in [`lean/QLF_Riemann.lean`](lean/QLF_Riemann.lean) has the form
...
- The `spectral_hilbert_polya` axiom could be reformulated as the proposed `MRE_bridge` (lines 313–344 of [`lean/QLF_Riemann.lean`](lean/QLF_Riemann.lean)) — same proof-theoretic strength, but expressed as the Mellin image of the per-event MRE saturation principle, making the substrate-to-analytic correspondence structurally motivated rather than ad-hoc.
