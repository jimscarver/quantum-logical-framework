...
**Executable demonstration:** [`braket_rho.py`](../../src/qlf/braket_rho.py) runs all of these identities numerically and checks them.

Here is the exact terminal input and output of the running `braket_rho.py` validation sandbox:

In the reorganized layout, run:

```bash
python -m qlf.braket_rho -repl
```
