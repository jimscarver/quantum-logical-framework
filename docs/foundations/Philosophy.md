...
The variational physics expression of this single axiom is S = ∫ℒ dΩ with ℒ = 0 — a null Lagrangian that is the condition of origin, not a filter. See [Lagrangian_Formulation.md](../formal/Lagrangian_Formulation.md) for the full variational treatment with machine-verified Lean theorem anchors.
...
python -m qlf.spacetime_dynamics          # watch ZFA events synthesize spacetime and drive expansion
