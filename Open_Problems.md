...
| **Mass spectrum from multiplicity** | 3rd-generation masses; τ-decay-vertex closure topology not yet pinned. **Partial handle:** Koide `Q=2/3` (read as the transverse-axis fraction) predicts `m_τ` from `m_e,m_μ` to **0.006%** (`Weak_Force.md §5a`, `koide_tau_demo.py`); deriving `Q=2/3`, the Koide angle, and the scale stays open | [`Standard_Model.md`](Standard_Model.md) §4.1, [`Weak_Force.md`](Weak_Force.md) §5a |
| **Weak sector (W/Z)** | Weak-isospin **SU(2) Lie algebra now machine-verified** in Σ₈ (`weak_isospin_su2`, `Q₈⊂SU(2)`); still open: `R_W`/`R_Z` (hence W/Z masses + Weinberg-angle value), coupling `g`, `G_F`, flavor-change & τ-decay vertex topology | [`Weak_Force.md`](Weak_Force.md), [`lean/BraKetRhoQuCalc.lean`](lean/BraKetRhoQuCalc.lean) |
