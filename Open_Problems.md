...
| **Gauge unification (forces from 3 axes)** | Structural **conjecture**: `dim(U(1)×SU(2)×SU(3)) = 12 = 1+3+8`, with `1+8 = 9 = N` (the α directional tensor, `u(3)` of the 3 axes) and `3 = su(2)` (verified weak). A dimension/structure *alignment*, not a derivation — no couplings, chirality, or `SU(3)` Lie-identification. Quark flavors/CKM still open | [`Forces_From_Three_Axes.md`](Forces_From_Three_Axes.md) |
