...
| **Charge vs `B−L`** | **Charge conservation ✅ Lean-anchored** (`signed_count_conserved`); every closure is neutral. **`B−L` is not a conserved signed count** — proved via `wcount_zero_on_ZFA` (conserved counts vanish on closures, but the deuteron has `B−L=1`; baryon vs antibaryon share a twist multiset yet `B−L=±1`). `B−L` is at most winding, and in the lepton sector it is **violated** (neutrino Majorana, `neutrino_majorana`) ⟹ 0νββ. So only the gauge charge is exact — QLF carries no exact global `B−L`. 🔵 Open: baryon-number winding as a positive Lean invariant (the turning-number candidate was tested & rejected, [`winding_invariant_demo.py`](winding_invariant_demo.py): it vanishes on the neutrino — spatial handedness ≠ lepton number) | [`lean/QLF_BMinusL.lean`](lean/QLF_BMinusL.lean), [`lean/QLF_Majorana.lean`](lean/QLF_Majorana.lean), [`Conservation.md`](Conservation.md) §8 |
