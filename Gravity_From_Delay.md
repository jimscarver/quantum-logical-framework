...
**The Planck *scale* is now forced; only its SI *value* and the matter depth remain.** It used to be said here that `L_Planck` is a substrate primitive "not derived from anything below." That overstated the gap: the Planck *scale* is now **derived as the minimal coherent-closure length** — the Compton–Schwarzschild self-dual point `μ²=1/2`, below which a would-be blanket is inside its own horizon and cannot close ([`QLF_PlanckScale`](lean/QLF_PlanckScale.lean): `coherent_iff_subplanck`, `planck_length_floor`, `planck_self_dual`; [`Planck_Scale.md`](Planck_Scale.md)). So the substrate granularity is *forced* by QLF's own emergent gravity + blanket closure, not posited. What genuinely remains: (i) the Planck length's **SI value in metres** is a *unit convention*, not a physics question — "what a metre is" cannot follow from logic; and (ii) **where the proton sits above the floor** (`R_p`) — the dimensional-transmutation hierarchy `ln R_p = 14π` ([`QLF_AlphaS`](lean/QLF_AlphaS.lean), 0.07%) with a residual few-% `M_Planck` calibration. The frequency distribution collapses the hierarchy from two numbers (`G` and `m_p`) to one (`R_p`); that one is the hierarchy problem — **separate from the granularity question, which is now settled**.
